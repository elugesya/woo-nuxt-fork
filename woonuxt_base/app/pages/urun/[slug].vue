      ...
      <div class="flex flex-col gap-10 md:flex-row md:justify-between lg:gap-24">
        ...
        <div class="lg:max-w-md xl:max-w-lg md:py-2 w-full">
          <div class="flex flex-col gap-4 mb-6">
            <h1 class="text-2xl md:text-4xl font-bold text-foreground tracking-tight leading-tight">
              {{ type.name }}
            ...
            <ProductPrice class="text-3xl md:text-4xl font-bold text-primary mt-2" :sale-price="type.salePrice" :regular-price="type.regularPrice" />
          </div>

          <div v-if="product.attributes && product.attributes.nodes && product.attributes.nodes.length" class="mb-8">
            <div class="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
              <template v-for="attr in product.attributes.nodes" :key="attr.id">
                <div v-if="(attr.options && attr.options.length) || (attr.terms && attr.terms.nodes && attr.terms.nodes.length)" class="flex flex-col bg-secondary/30 p-3 rounded-lg border border-border/50">
